...
def find_val_extreme(station_id):
    ...
    rainy_days = ds_search['prcp'].where(ds_search['prcp'] > 1, drop=True).sel(time=slice(start_date,end_date))
...
df_neighbour = df[(df["Start_Year"]<=1960) &(df["End_Year"]>=2023)]
## for stations to be a station to do semi-variogram
df_spec = df[(df["Start_Year"]<=1960) &(df["End_Year"]>=2023)]
...
for spec_id in id_list:
    csv_file = f"./data/seasonal_p90_500km_1960/{spec_id}_station_moving_list_all_events.csv"
    ...
    if not os.path.exists(csv_file):
        ...
        if len(df_search_in)<20:
            ...
            var = "prcp"
            ds_sel = ds_spec.sel(time=slice(start_date,end_date))
